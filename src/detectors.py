...
def _prompt_from_call(node: ast.AST, file: str, imports: dict[str, str]) -> Surface | None:
    ...
    return Surface(PROMPT_TEMPLATE, call_name(node), file, node.lineno,
...
def _prompt_from_assignment(node: ast.AST, file: str) -> Surface | None:
    ...
    if not shape:
        return None
    return Surface(PROMPT_TEMPLATE, name, file, node.lineno, detail=f"prompt {shape} assigned to {name}")
...
def _agent_from_call(node: ast.AST, file: str, imports: dict[str, str]) -> Surface | None:
    ...
    name = call_name(node)
    return Surface(AGENT_DEF, name, file, node.lineno, detail=detail, module=imports.get(root, ""))
...
def _tool_from_function(node: ast.AST, file: str) -> Surface | None:
    ...
    if not matched:
        return None
    return Surface(TOOL_CALL, node.name, file, node.lineno, detail=f"function decorated with @{matched[0]}")
...
def _tool_from_call(node: ast.AST, file: str, imports: dict[str, str]) -> Surface | None:
    ...
    return Surface(TOOL_CALL, name, file, node.lineno, detail=detail, module=imports.get(root, ""))
...
def _tool_from_class(node: ast.AST, file: str) -> Surface | None:
    ...
    if not matched:
        return None
    return Surface(TOOL_CALL, node.name, file, node.lineno, detail=f"class subclassing {matched[0]}")
...
def _source_from_call(node: ast.AST, file: str, imports: dict[str, str]) -> Surface | None:
    ...
    if name in DATA_SOURCE_CALLS:
        return Surface(DATA_SOURCE, name, file, node.lineno, detail=DATA_SOURCE_CALLS[name], module=module)
    leaf = call_leaf(node)
    if leaf not in DATA_SOURCE_METHODS:
        return None
    return Surface(DATA_SOURCE, name, file, node.lineno, detail=DATA_SOURCE_METHODS[leaf], module=module)
...
def _source_from_route(node: ast.AST, file: str) -> Surface | None:
    ...
    for decorator in decorator_names(node):
        ...
        if root in ROUTE_DECORATOR_ROOTS and method in HTTP_METHODS:
            ...
            return Surface(DATA_SOURCE, node.name, file, node.lineno, detail=f"http {shape} input")
    return None
