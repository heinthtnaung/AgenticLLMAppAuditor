...
def _read(path: Path, what: str) -> dict:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            ...
            "Run the auditor over this app first."
        )
    try:
...
def load_app(app: str, artifacts_dir: Path) -> tuple[dict, dict, dict]:
    ...
    return (
        ...
        _read(artifacts_dir / app / FINDINGS_NAME, f"{app}'s findings"),
        _read(artifacts_dir / app / SURFACES_NAME, f"{app}'s surfaces"),
    )
...
def score_apps(apps: list[str], artifacts_dir: Path,
    ...
    scored = [score_app(app, *load_app(app, artifacts_dir)) for app in sorted(apps)]
...
def write_evaluation(document: dict, artifacts_dir: Path) -> Path:
    ...
    One file for the whole run, so it sits in `artifacts/` rather than under
    any one app: a comparison across apps is not a per-app fact.
    """
