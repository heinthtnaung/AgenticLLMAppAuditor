...
`check_key` is pure, so every case here is a dict.

`check_key` is private and tested directly on purpose: each message below is
...
from evaluation.harness import KEY_FIELDS, KEY_SCHEMA_VERSION, check_key
