...
# The Python corpus SBOM, component by component. Recorded before `locked`
...
def build_widget(constraint: str, version: str | None, manifests: list[str],
    ...
    if version is not None:
        components = [{"type": "library", "name": "widget", "version": version}]
...
def test_a_python_app_shipping_a_poetry_lock_gets_locked_versions() -> None:
    ...
    keyed on the ecosystem nothing in the corpus would catch it.
    """
...
def test_a_yarn_lock_makes_the_npm_version_a_fact() -> None:
    """Add the lockfile and the same npm range resolves to a versioned purl."""
    component = build_widget("^1.2.3", "1.2.3", [NPM_MANIFEST, YARN_LOCK], NPM)
    assert component["version_source"] == LOCKED
    assert component["purl"] == "pkg:npm/widget@1.2.3"


def test_the_js_corpus_app_locks_every_version_its_lockfile_resolved() -> None:
...
def test_the_python_corpus_sbom_is_otherwise_exactly_what_it_was() -> None:
    ...
    built = [(c["name"], c["version"], c["version_source"], c["purl"])
             for c in corpus_sbom()["components"]]
    assert built == PYTHON_CORPUS_COMPONENTS


def test_no_python_corpus_component_is_locked() -> None:
    """Said directly, so the reason the tuples above must not change is on the record."""
    sources = [c["version_source"] for c in corpus_sbom()["components"]]
