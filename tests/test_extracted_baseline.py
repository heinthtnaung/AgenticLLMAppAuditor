...
import pytest
from conftest import CORPUS_APPS, CORPUS_DIR
from extractor import extract_repo

BASELINE_NAME = "extracted_baseline.json"


def baseline(app: str) -> dict:
    """Read one app's recorded extractor output."""
    return json.loads((CORPUS_DIR / app / BASELINE_NAME).read_text(encoding="utf-8"))
