...
from .audit_stub import (
    APP, RUN_SECONDS, URL, UNEXPECTED_ERROR, artifacts_dir_for, open_a_store,
...
# The seven keys of the result envelope, which is `docs/SCHEMAS.md`'s "a view,
# not an artifact" and is stored whole.
...
def run_to_completion(registry: Registry, **options) -> tuple[RunRecord, dict | None]:
    ...
    accepted = registry.start(a_request(**options))
...
def test_a_finished_run_stores_the_documented_envelope(monkeypatch, tmp_path) -> None:
    """Seven keys, kept so a past run stays viewable after `artifacts/` is cleaned."""
    stub_the_audit(monkeypatch, tmp_path)
    plant_documents(tmp_path)
